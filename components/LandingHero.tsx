...
export function LandingHero() {
  return (
    <section className="space-y-8">
      <div className="inline-flex items-center gap-2 rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-sky-200">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Link
          href="/"
          className="rounded-xl bg-brand-500 px-6 py-3 text-base font-semibold text-slate-950 shadow-lg shadow-brand-500/30 transition hover:-translate-y-[1px] hover:bg-brand-400"
        >
          Start mapping now
        </Link>
        <Link
        ...
        <div className="rounded-full border border-slate-800 px-4 py-2 text-xs font-semibold text-slate-300">
          Use coupon <span className="text-brand-300">DECODE20</span> on Pro or Team
        </div>
      </div>
    </section>
  );
