              ...
              <div className="space-y-2" data-cf-list="true" ref={(el) => {
                ...
                if (el && !(el as any).__cfInit) {
                  ...
                  el.addEventListener("mousedown", (ev) => {
                    (el as any).__dragFromHandle = !!(ev.target as HTMLElement).closest?.("[data-cf-handle]");
                  }, true);
                }
              }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex items-center gap-1.5 px-2 py-1.5 ${isLight ? "border-b border-slate-100" : "border-b border-slate-700/20"}`}>
                        {/* Drag handle */}
                        <div data-cf-handle="true" className="flex flex-col gap-0 cursor-grab active:cursor-grabbing opacity-40 hover:opacity-80 transition p-0.5 rounded hover:bg-slate-500/10">
                          <svg className="h-4 w-4 text-slate-400" viewBox="0 0 24 24" fill="currentColor"><circle cx="9" cy="6" r="1.8"/><circle cx="15" cy="6" r="1.8"/><circle cx="9" cy="12" r="1.8"/><circle cx="15" cy="12" r="1.8"/><circle cx="9" cy="18" r="1.8"/><circle cx="15" cy="18" r="1.8"/></svg>
                        </div>
                        {/* Type icon */}
                        ...
                        {/* Type selector */}
                        <select
                          className={`border rounded-md px-1.5 py-0.5 text-[10px] font-medium cursor-pointer flex-shrink-0 ${isLight ? "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100" : "border-slate-700/30 bg-slate-800/40 text-slate-400 hover:bg-slate-700/50"}`}
                          value={field.type}
                          onChange={(e) => {
                        ...
                        <button
                          type="button"
                          onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); removeField(); }}
                          className={`p-1 rounded transition flex-shrink-0 ${isLight ? "text-slate-400 hover:text-red-500 hover:bg-red-50" : "text-slate-500 hover:text-red-400 hover:bg-red-500/10"}`}
